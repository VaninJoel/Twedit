...
# # #         self.pm.unloadPlugins()
    ...
    def saveSettingsOpenFilesToRestore(self,openFilesToRestore):
        ...
        dbgMsg("saving fileList.count()=",len(fileList)   )
    ...
    def restoreFindAndReplaceHistory(self,_frh):
        ...
        for i in xrange(len(findHistoryList)):
        ...
        for i in xrange(len(replaceHistoryList)):
        ...
        for i in xrange(len(filtersHistoryList)):
        ...
        for i in xrange(len(directoryHistoryList)):
        ...
        for editor in self.getEditorList():
            ...
            try:
                ...
                if not editor.isModified():
                    ...
                    ret = QtWidgets.QMessageBox.warning(self, "Missing File",message,QtWidgets.QMessageBox.Yes |QtWidgets.QMessageBox.No)
    ...
    def saveAs(self,suggestedName=None,_editor=None):
        ...
        if suggestedName is None or  isinstance(suggestedName, bool) : # saveAs is called by default with False bool type argument and we need to make sure that it is not converted to int index
            ...
            if sys.platform=='darwin':            
                fileName=QtWidgets.QFileDialog.getSaveFileName(self,"Save File",currentFilePath,self.fileDialogFilters)
            else:
                fileName=QtWidgets.QFileDialog.getSaveFileName(self,"Save File",currentFilePath,currentFilterString+";;"+self.fileDialogFilters)            
            ...
            dbgMsg("")
            fileName = QtWidgets.QFileDialog.getSaveFileName(self,"Save File",suggestedName)
            if fileName=="":
                return False            
...
#             menuOwnerObj.connect(action,SIGNAL("triggered()"),_recentItemSlot)
    ...
    def createActions(self):
        ...
        self.printAct = QtWidgets.QAction(QtGui.QIcon(':/icons/document-print.png'),"Print...", self,shortcut="Ctrl+P",statusTip="Print current document",triggered=self.printCurrentDocument)                
        ...
        self.closeAllButCurrentAct = QtWidgets.QAction("Close All But Current Tab", self,statusTip="Close all tabs except current one",triggered=self.__closeAllButCurrent)                        
        ...
        self.deleteDocAct = QtWidgets.QAction("Delete from disk", self,statusTip="Delete current document from disk",triggered=self.__deleteCurrentDocument)                
        am.addAction(self.deleteDocAct)            
        
        self.movetoOtherView = QtWidgets.QAction("Move To Other View", self,statusTip="Move document to new panel",triggered=self.__moveToOtherView)                        
        ...
        self.foldAllAct = QtWidgets.QAction("Toggle Fold All", self, statusTip="Fold document at all folding points", triggered=self.foldAll)  
...
#         self.connect(self.showEOLAct,    SIGNAL('triggered(bool)'),  self.showEOL)
        ...
        self.showTabGuidelinesAct = QtWidgets.QAction("Show Tab Guidelines", self, statusTip="Show Tab guidelines in a current document")  
...
#         self.connect(self.showTabGuidelinesAct,    SIGNAL('triggered(bool)'),  self.showTabGuidelines)
        ...
        self.showLineNumbersAct = QtWidgets.QAction("Show Line Numbers", self, statusTip="Show line numbers in a current document")  
...
#         self.connect(self.showLineNumbersAct,    SIGNAL('triggered(bool)'),  self.showLineNumbers)
        ...
        self.showFindInFilesDockAct=QtWidgets.QAction("Show Find in Files Results", self, statusTip="Show Find in Files Results", triggered=self.toggleFindInFilesDock)
        ...
        self.toggleBookmarkAct = QtWidgets.QAction(QtGui.QIcon(':/icons/flag.png'),"Toggle Bookmark", self, shortcut="Ctrl+F2",
        ...
        self.increaseIndentAct = QtWidgets.QAction(QtGui.QIcon(':/icons/format-indent-more.png'),"Increase Indent", self, shortcut="Tab",
                statusTip="Increase Indent", triggered=self.increaseIndent)  
        am.addAction(self.increaseIndentAct)                        
                
        self.decreaseIndentAct = QtWidgets.QAction(QtGui.QIcon(':/icons/format-indent-less.png'),"Decrease Indent", self, shortcut="Shift+Tab",
        ...
        self.convertEOLWinAct=QtWidgets.QAction("Windows EOL", self,statusTip="Convert End of Line Character to Windows Style",triggered=self.convertEolWindows) 
        am.addAction(self.convertEOLWinAct)                                                
        self.convertEOLUnixAct=QtWidgets.QAction("Unix EOL", self,statusTip="Convert End of Line Character to Unix Style",triggered=self.convertEolUnix) 
        am.addAction(self.convertEOLUnixAct)
        self.convertEOLMacAct=QtWidgets.QAction("Mac OS 9 EOL", self,statusTip="Convert End of Line Character to Mac Style",triggered=self.convertEolMac)        
...
#             fileName=str(action.data().toString())    
    ...
    def loadFiles(self, fileNames):
        ...
        for fileName in fileNames:      
            ...
            self.loadFile(os.path.abspath(fileName))# "normalizing" file name to make sure \ and / are used in a consistent manner            
...
#         removalSuccesful=stringList.removeAll(_itemName)
        stringList = filter(lambda a: a != _itemName, stringList)        
